...
def get_cord(xml_adr, _dict):
    '''
    xml parser
    return [ [cate, [xmin,ymin,xmax,ymax]], [cate, box], ... ]
    '''
    ...
    for child in root.findall("object"):
        ...
        ret.append([cate_id, box])
    return (width, height), ret


def normalize_convert(size, box):
    '''
    XML [xmin,ymin,xmax,ymax] transfer to darknet LABELS format data [ x_center, y_center, w, h ] * ratio
    '''
    dw = np.float32(1./size[0])
    dh = np.float32(1./size[1])
    x = np.float32((box[0] + box[2])/2.0)
    y = np.float32((box[1] + box[3])/2.0)
...
def build_targets(model, targets):
    ...
    iou_thresh = model.lwhyp['ignore_thresh']  # hyperparameter
    # if type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel):
    #     model = model.module
    multi_gpus = type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)

    nt = len(targets)
    txy, twh, tcls, tbox, indices, anchor_vec = [], [], [], [], [], []
    # print ("&&&&&& check build_targets:", dir(model))
    for i in model.yolo_layers:
        if multi_gpus:
            layer = model.module.module_list[i][0]
        else:
            layer = model.module_list[i][0]

        print (">>>>>> module_list", model.module_list[i])
        print (">>>>>> layer", layer)
        print ('%$%%%%%%% check build_targets:', dir(layer))
...
def compute_loss_v01(p, targets, model):  # predictions, targets, model
    ft = torch.cuda.FloatTensor if p[0].is_cuda else torch.Tensor
...
def build_targets_v01(model, targets):
    # targets = [image, class, x, y, w, h]
    iou_thresh = model.lwhyp['iou_thresh']  # hyperparameter
    if type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel):
        model = model.module
...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.5, nms_style='MERGE'):
    """
        ...
        for c in pred[:, -1].unique():
            ...
            if nms_style == 'OR':  # default
                ...
                while len(dc):
                    ...
                    # dc = dc[dc[:, 4] > nms_thres]  # (under developed) new line per https://github.com/ultralytics/yolov3/issues/362
...
def plot_results_v01(start=1, stop=0):  # from utils.utils import *; plot_results()
    # Plot training results files 'results*.txt'
    # import os; os.system('wget https://storage.googleapis.com/ultralytics/yolov3/results_v3.txt')
