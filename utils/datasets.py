...
class LoadImagesAndLabels(Dataset):  
    ...
    '''
    def __init__(self, filepath, img_size=416, batch_size=16, augment=False, _format="darknet", _dict=None):
        ...
        if not self.isxml:
            ...
            self.label_files = [
                x.replace('JPEGImages', 'Annotations').replace('.bmp', '.xml').replace('.jpg', '.xml').replace('.png', '.xml')
                for x in self.img_files]
        ...
        for i, file in enumerate(iter):
            try:
                if not self.isxml:
                    ...
                    # print("DataLoader check:", file, self.labels[i])
                else:
                    (width, height), rets = get_cord(file, _dict)
                    _labels = []
                    if width !=512 or height !=512:
                        print(file, width, height)
                    for ret in rets:
                        cate_id, box = ret
                        _x,_y,_w,_h = normalize_convert((width, height), box)
                        _labels.append([cate_id, _x, _y, _w, _h])
                    self.labels[i] = np.array(_labels, dtype=np.float32)
                    # print("DataLoader check:", file, self.labels[i])
            except:  # missing label file
                print(">>> *** >>> Error Occur in `LoadImagesAndLabels.__init__: cannot Load annotations correctly`")
                pass
